...
##3.3 内存上限控制
  ...
  **默认情况下，容器可以使用主机上的所有空闲内存**。
  ...
  docker在解释-m参数时，除了容器可以使用的内存外，还**默认为容器分配了同样大小的swap内存空间**，也就是说，上面的命令创建出的容器实际上最多可以使用256m内存，而不是128m内存。
...
-rw-r--r-- 1 root root 0 Mar 30 18:18 tasks  
 ...
 上述所有文件的含义，同样可以参考RedHat的文档[Resource_Management_Guide](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/6/html/Resource_Management_Guide/index.html)。
...
268435456
 ...
 文件的值正好为256m（256*1024*1024=268435456）。
#4. docker的存储资源
##4.1 镜像与容器的存储结构
##4.2 卷、绑定与数据卷
   动态绑定卷组的实现
